...
const bookingSchema = new mongoose.Schema(
    {
        ...
        paymentInterval: {
            ...
            enum: {
                ...
                message:
                    'paymentInterval can only either be hourly, daily, weekly, bi-weekly, monthly, or full',
            },
            lowercase: true,
        },
        ...
        lodgeEndDate: {
            ...
            validate: {
                ...
                message:
                    'Booking lodge end date should be at least one day ahead of the start date',
            },
        },
        ...
        history: [
            {
                ...
                status: {
                    ...
                    enum: {
                        ...
                        message:
                            'booking status can only either be failed or successful',
                    },
                    lowercase: true,
                },
            },
...
bookingSchema.virtual('paymentPercentage').get(function () {
    return Math.ceil((this.totalAmountPaid / this.totalAmount) * 100);
});

bookingSchema.virtual('daysLeft').get(function () {
    const currentDate = Date.now();
    const lodgeStartDate = new Date(this.lodgeStartDate);

    if (currentDate < lodgeStartDate) {
        const diffTime = Math.abs(currentDate - this.lodgeStartDate);
        const diffDate = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        return diffDate;
    }
