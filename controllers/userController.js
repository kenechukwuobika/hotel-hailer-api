...
exports.setVendorId = (type = 'filter') =>
    catchAsync(async (req, res, next) => {
        if (req.user && req.user.role === 'vendor') {
            if (type === 'body') {
                req.body.vendor = req.user._id;
            } else if (type === 'filter') {
                const customFilter = req.customFilter
                    ? { ...req.customFilter }
                    : {};
...
exports.updateMe = catchAsync(async (req, res, next) => {
    ...
    if (bookings.length !== 0) {
        return next(
            new AppException(
                401,
                'You cannot update your profile if you have an active booking.'
            )
        );
    }
...
exports.deleteMe = catchAsync(async (req, res, next) => {
    ...
    if (bookings) {
        return next(
            new AppException(
                401,
                'You cannot delete your profile if you have an active booking.'
            )
        );
    }
