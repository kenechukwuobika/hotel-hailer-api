...
const handleUniqueError = (error) => {
    ...
    if (keyValues.includes('property') && keyValues.includes('user')) {
        return new AppException(
            400,
            `Sorry you have already submitted a review on this property`
        );
    }
    ...
    return new AppException(
        ...
        `this ${Object.keys(
            error.keyValue
        )} is already being used by another customer`
    );
};
...
const handleJWTError = () => {
    return new AppException(
        401,
        'Invalid token!!!, please login again to get a token'
    );
};
...
const handleJWTExpireError = () => {
    return new AppException(
        401,
        'token has expired, please login again to get a token'
    );
};
