...
const validateBooking = async (req, next, mode='pay') => {
    try {
        ...
        const { cardId, lodgeEndDate, lodgeStartDate, paymentInterval } = req.body;
        const { user } = req;
        const currentDate = new Date(Date.now());
...
exports.getBookingPlans = catchAsync(async (req, res, next) => {
    ...
    if(!propertyId){
        return next(new AppException(400, "Invalid input. Please provide the property Id"));
    }
...
exports.initializeTransaction = catchAsync( async (req, res, next) => {
    const { params, booking } = await validateBooking(req, next, mode="initialize");    
	try {
...
exports.pay = catchAsync( async (req, res, next) => {
    const { params, booking, user } = await validateBooking(req, next, mode="pay");    
