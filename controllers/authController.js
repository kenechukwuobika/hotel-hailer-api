...
exports.signup = catchAsync(async (req, res, next) => {
    if (req.vendor && req.vendor.role === 'admin') {
        return next(
            new AppException(400, "Sorry, you can't signup as an admin")
        );
    }
    ...
    if (newUser.role === 'vendor') {
        ...
        if (!wallet)
            return next(
                new AppException(
                    404,
                    `Could not create wallet for user with id: ${newUser._id}}`
                )
            );
    }
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (Object.keys(filter).length === 0 || !password) {
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!currentUser) {
        return next(new AppException(401, 'User does not exist'));
    }
...
const checkFieldExist = (field) =>
    catchAsync(async (req, res, next) => {
        ...
        if (user) {
            return next(
                new AppException(400, `${field} is in use by another user`)
            );
        }
